      ...
      <div className="flex justify-center mb-6">
        ...
        >
          {participants.map((p, i) => (
            <option key={p.id} value={i}>{p.cosplayName}</option>
          ))}
        </select>
      </div>
      ...
      <AppBar position="fixed" sx={{
        ...
        <Toolbar sx={{ display: "flex", gap: 2, justifyContent: "center", alignItems: "center", py: 1.5 }}>
          ...
          <button onClick={submitRatings}
            className={`ml-auto px-5 py-2 rounded font-semibold transition ${darkMode ? "bg-green-600 hover:bg-green-500 text-white" : "bg-green-600 hover:bg-green-700 text-white"}`}>
            Punkte absenden
          </button>
        </Toolbar>
      </AppBar>
